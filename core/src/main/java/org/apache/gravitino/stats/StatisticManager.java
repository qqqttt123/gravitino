...
public class StatisticManager {
      ...
      return store.relationOperations()
          ...
          .map(
              entity -> {
                ...
                String name = statisticEntity.fields().get(StatisticEntity.NAME).toString();
                StatisticValue<?> value =
                    (StatisticValue<?>) statisticEntity.fields().get(StatisticEntity.VALUE);
                return new StatisticImpl(name, value);
              })
  ...
  public List<Statistic> updateStatistics(
    ...
    try {
      ...
      return statistics.entrySet().stream()
          .map(entry -> new StatisticImpl(entry.getKey(), entry.getValue()))
          .collect(Collectors.toList());
    } catch (NoSuchEntityException nse) {
      LOG.warn(
  ...
  private static class StatisticImpl implements Statistic {
    ...
    StatisticImpl(String name, StatisticValue<?> value) {
      this.name = name;
      this.value = value;
    }
